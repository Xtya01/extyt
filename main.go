...
func downloadDBFromTelegram(fileID string) error {
	...
	defer resp.Body.Close()
	var gf struct{ Ok bool; Result struct{ FilePath string `json:"file_path"` } `json:"result"` }
	json.NewDecoder(resp.Body).Decode(&gf)
	if !gf.Ok { return fmt.Errorf("getFile fail") }
...
func getTelegramFilePath(fileID string) string {
	...
	if err != nil { return "" }
	defer resp.Body.Close()
	var res struct{ Ok bool; Result struct{ FilePath string `json:"file_path"` } `json:"result"` }
	json.NewDecoder(resp.Body).Decode(&res)
	if res.Ok { return res.Result.FilePath }
	return ""
}

// ---------- COOKIES FIX - YT bot check ke liye ----------
func getCookiesArg() []string {
	// 1. YT_COOKIES_B64 ENV se (most reliable Koyeb pe)
	if b64 := os.Getenv("YT_COOKIES_B64"); b64 != "" {
		b64 = strings.TrimSpace(b64)
		// newlines hata do jo base64 site add karti hai
	...
	// 2. Direct YT_COOKIES ENV (raw text)
...
func health(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte(fmt.Sprintf("OK v7 - %d songs - cookies: %v", c, hasCookies)))
}
	...
	if token != "" && chatID != "" {
		if f, err := os.Open(tmpFile); err == nil {
			...
			if resp, err := http.DefaultClient.Do(req); err == nil {
				...
				var res struct {
					Ok bool `json:"ok"`
					Result struct{ Audio struct{ FileID string `json:"file_id"` } `json:"audio"` } `json:"result"`
				}
				json.NewDecoder(resp.Body).Decode(&res)
				if res.Ok {
...
func main() {
	...
	http.HandleFunc("/convert", playHandler) // backward compat
