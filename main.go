...
func getCookiesContent() string{
	if b64:=os.Getenv("YT_COOKIES_B64"); strings.TrimSpace(b64)!=""{
		...
		if d,err:=base64.StdEncoding.DecodeString(trimmed); err==nil{
			log.Println("Using YT_COOKIES_B64 std",len(d)); return string(d)
		}
		if d,err:=base64.RawStdEncoding.DecodeString(trimmed); err==nil{
			log.Println("Using YT_COOKIES_B64 raw",len(d)); return string(d)
		}
		if d,err:=base64.URLEncoding.DecodeString(trimmed); err==nil{
			log.Println("Using YT_COOKIES_B64 url",len(d)); return string(d)
		}
		return trimmed
	}
	...
	if strings.Contains(trimmed,"Netscape")||strings.Contains(trimmed,"# Netscape"){
		log.Println("Using YT_COOKIES raw netscape"); return trimmed
	}
	if d,err:=base64.StdEncoding.DecodeString(trimmed); err==nil{
		s:=string(d); if strings.Contains(s,"Netscape")||strings.Contains(s,"youtube"){log.Println("YT_COOKIES detected base64"); return s}
	}
	if d,err:=base64.RawStdEncoding.DecodeString(trimmed); err==nil{
		s:=string(d); if strings.Contains(s,"Netscape")||strings.Contains(s,"youtube"){log.Println("YT_COOKIES detected base64 raw"); return s}
	}
	log.Println("Using YT_COOKIES raw"); return trimmed
}
func initUsers(){
	...
	if cfg.SubUsersRaw!=""{
		for _,p:=range strings.Split(cfg.SubUsersRaw,","){
			...
			kv:=strings.SplitN(p,":",2); if len(kv)==2{usersMap[strings.TrimSpace(kv[0])]=strings.TrimSpace(kv[1])}
		}
	}
...
func telegramUploadAudioFile(filePathOrUrl string, song *Song)(string,string,error){
	...
	if strings.HasPrefix(filePathOrUrl,"http"){
		resp,err:=httpClient.Get(filePathOrUrl); if err!=nil{return "","",err}; defer resp.Body.Close()
		data,_=io.ReadAll(io.LimitReader(resp.Body,48*1024*1024))
	} else {
		f,err:=os.Open(filePathOrUrl); if err!=nil{return "","",err}; defer f.Close()
		data,_=io.ReadAll(io.LimitReader(f,48*1024*1024))
	}
	body:=&bytes.Buffer{}; w:=multipart.NewWriter(body); w.WriteField("chat_id",cfg.TelegramMusicChatID)
	w.WriteField("caption",fmt.Sprintf("🎵 %s - %s | %s",song.Artist,song.Title,song.ID))
	part,_:=w.CreateFormFile("audio",filename); part.Write(data); w.Close()
	apiUrl:=fmt.Sprintf("https://api.telegram.org/bot%s/sendAudio",cfg.TelegramToken)
	req,_:=http.NewRequest("POST",apiUrl,body); req.Header.Set("Content-Type",w.FormDataContentType())
	client:=&http.Client{Timeout:60*time.Second}; resp,err:=client.Do(req); if err!=nil{return "","",err}; defer resp.Body.Close()
	b,_:=io.ReadAll(resp.Body)
	var res struct{Ok bool; Result struct{Audio struct{FileID string `json:"file_id"`; FileUniqueID string `json:"file_unique_id"`} `json:"audio"`; Document struct{FileID string `json:"file_id"`; FileUniqueID string `json:"file_unique_id"`} `json:"document"`} `json:"result"`; Description string `json:"description"`}
	json.Unmarshal(b,&res)
	if !res.Ok{
		body2:=&bytes.Buffer{}; w2:=multipart.NewWriter(body2); w2.WriteField("chat_id",cfg.TelegramMusicChatID)
		w2.WriteField("caption",fmt.Sprintf("🎵 %s - %s | %s",song.Artist,song.Title,song.ID))
		part2,_:=w2.CreateFormFile("document",filename); part2.Write(data); w2.Close()
		apiUrl2:=fmt.Sprintf("https://api.telegram.org/bot%s/sendDocument",cfg.TelegramToken)
		req2,_:=http.NewRequest("POST",apiUrl2,body2); req2.Header.Set("Content-Type",w2.FormDataContentType())
		resp2,err2:=client.Do(req2); if err2!=nil{return "","",fmt.Errorf("audio fail: %s doc err: %v",res.Description,err2)}; defer resp2.Body.Close()
		...
		json.Unmarshal(b2,&res2); if !res2.Ok{return "","",fmt.Errorf("tg upload failed audio:%s doc:%s",res.Description,res2.Description)}
		return res2.Result.Document.FileID,res2.Result.Document.FileUniqueID,nil
	}
	fid:=res.Result.Audio.FileID; if fid==""{fid=res.Result.Document.FileID}
	fuid:=res.Result.Audio.FileUniqueID; if fuid==""{fuid=res.Result.Document.FileUniqueID}
	if fid==""{return "","",fmt.Errorf("no file_id in response %s",string(b))}
	return fid,fuid,nil
}
func cacheSongToTelegram(s *Song, streamUrl string){
	if s.TgFileID!=""{return}; if _,inProg:=tgCacheInProgress.Load(s.ID); inProg{return}
	tgCacheInProgress.Store(s.ID,true); defer tgCacheInProgress.Delete(s.ID)
	log.Printf("Caching to TG: %s - %s (%s)",s.Artist,s.Title,s.ID)
	fid,fuid,err:=telegramUploadAudioFile(streamUrl,s)
	if err!=nil{log.Printf("TG cache failed for %s: %v",s.ID,err); return}
	db.Lock(); if song,ok:=db.Songs[s.ID]; ok{song.TgFileID=fid; song.TgFileUniqueID=fuid; song.CachedAt=time.Now().Format(time.RFC3339)}; db.Unlock()
	db.save(); go telegramUploadDB(); log.Printf("Cached %s to TG file_id %s",s.ID,fid[:20])
}
...
func importYoutubePlaylist(pid string,owner string)(*Playlist,error){
	...
	pl:=&Playlist{ID:plID,Name:title,Owner:owner,Comment:fmt.Sprintf("YT Import %d songs - %s - TG Cache",len(allIDs),owner),SongIDs:[]string{},Created:time.Now().Format(time.RFC3339),Source:"yt",SourceURL:"https://www.youtube.com/playlist?list="+pid}
...
func handleGetUser(w http.ResponseWriter,r *http.Request){ok,user:=checkAuth(r); if !ok{writeJSON(w,200,subFail("auth",40)); return}; respond(w,r,map[string]interface{}{"user":map[string]interface{}{"username":user,"adminRole":user==cfg.SubUser,"playlistRole":true,"streamRole":true,"downloadRole":true}})}
...
func songToSubsonic(s *Song,r *http.Request)map[string]interface{}{starred:=false; if r!=nil{if ok,user:=checkAuth(r); ok{db.RLock(); if m,ok:=db.Starred[user]; ok{if m[s.ID]{starred=true}}; db.RUnlock()}}; m:=map[string]interface{}{"id":s.ID,"title":s.Title,"artist":s.Artist,"album":s.Album,"albumId":s.AlbumID,"artistId":s.ArtistID,"coverArt":s.CoverArt,"duration":s.Duration,"contentType":"audio/mp4","suffix":"m4a","year":s.Year,"genre":s.Genre,"playCount":s.PlayCount}; if starred{m["starred"]=time.Now().Format(time.RFC3339)}; if s.TgFileID!=""{m["cached"]="tg"}; return m}
...
func handleStream(w http.ResponseWriter,r *http.Request){
	...
	if s.TgFileID!=""{if tgUrl,err:=telegramGetFileUrl(s.TgFileID); err==nil{req,_:=http.NewRequest("GET",tgUrl,nil); resp,err:=httpClient.Do(req); if err==nil{defer resp.Body.Close(); w.Header().Set("Content-Type","audio/mp4"); w.Header().Set("X-Cache","TG"); io.Copy(w,resp.Body); return}}}
	...
	req,_:=http.NewRequest("GET",urlStr,nil); req.Header.Set("User-Agent","Mozilla/5.0"); resp,err:=httpClient.Do(req); if err!=nil{http.Error(w,"upstream",502); return}; defer resp.Body.Close(); w.Header().Set("Content-Type","audio/mp4"); w.Header().Set("X-Cache","MISS"); io.Copy(w,resp.Body)
}
func handleDownload(w http.ResponseWriter,r *http.Request){
	...
	w.Header().Set("Content-Type","audio/mp4"); w.Header().Set("Content-Disposition",fmt.Sprintf("attachment; filename=\"%s - %s.m4a\"",s.Artist,s.Title)); io.Copy(w,resp.Body)
}
func handleCoverArt(w http.ResponseWriter,r *http.Request){id:=r.URL.Query().Get("id"); var thumb string; db.RLock(); if al,ok:=db.Albums[id]; ok{thumb=al.ThumbURL}else if s,ok:=db.Songs[id]; ok{thumb=s.ThumbURL}; db.RUnlock(); if thumb==""{db.RLock(); for _,al:=range db.Albums{if al.ID==id{thumb=al.ThumbURL; break}}; for _,s:=range db.Songs{if s.ID==id{thumb=s.ThumbURL; break}}; db.RUnlock()}; if thumb==""{http.Redirect(w,r,"https://via.placeholder.com/500x500.png?text=No+Art",302); return}; resp,_:=httpClient.Get(thumb); if resp==nil{http.Redirect(w,r,thumb,302); return}; defer resp.Body.Close(); w.Header().Set("Content-Type","image/jpeg"); w.Header().Set("Cache-Control","public, max-age=86400"); io.Copy(w,resp.Body)}
func handleGetPlaylists(w http.ResponseWriter,r *http.Request){ok,user:=checkAuth(r); if !ok{writeJSON(w,200,subFail("auth",40)); return}; db.RLock(); defer db.RUnlock(); var list []map[string]interface{}; for _,pl:=range db.Playlists{if pl.Owner==user||pl.Public||user==cfg.SubUser{cachedCount:=0; for _,sid:=range pl.SongIDs{if s,ok:=db.Songs[sid]; ok&&s.TgFileID!=""{cachedCount++}}; list=append(list,map[string]interface{}{"id":pl.ID,"name":pl.Name,"owner":pl.Owner,"songCount":len(pl.SongIDs),"created":pl.Created,"public":pl.Public,"comment":pl.Comment,"cachedCount":cachedCount})}}; respond(w,r,map[string]interface{}{"playlists":map[string]interface{}{"playlist":list}})}
func handleGetPlaylist(w http.ResponseWriter,r *http.Request){if ok,_:=checkAuth(r); !ok{writeJSON(w,200,subFail("auth",40)); return}; id:=r.URL.Query().Get("id"); db.RLock(); pl,ok:=db.Playlists[id]; db.RUnlock(); if !ok{writeJSON(w,200,subFail("not found",70)); return}; db.RLock(); var songs []map[string]interface{}; for _,sid:=range pl.SongIDs{if s,ok:=db.Songs[sid]; ok{songs=append(songs,songToSubsonic(s,r))}}; db.RUnlock(); respond(w,r,map[string]interface{}{"playlist":map[string]interface{}{"id":pl.ID,"name":pl.Name,"owner":pl.Owner,"songCount":len(songs),"entry":songs,"created":pl.Created,"public":pl.Public,"comment":pl.Comment}})}
...
func main(){
	...
	cookieContent:=getCookiesContent(); if cookieContent!=""{os.WriteFile("/tmp/cookies.txt",[]byte(cookieContent),0600); log.Printf("Cookies written %d bytes",len(cookieContent))} else {log.Println("No cookies")}
